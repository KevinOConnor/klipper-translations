...
もしKlipperの設定ディレクトリに適切なプリンタ設定ファイルがなければ、プリンタ製造元のウェブサイトを探して適切なKlipperの設定ファイルがあるかどうか確かめてください。
...
Klipperは3Dプリンタのファームウェアなので、ユーザーがそれと対話するには何らかの方法が必要です。

現在の最良の選択は [Moonraker web API](https://moonraker.readthedocs.io/) を通して情報を回収するフロントエンドです、他にもオプションとして、[Octoprint](https://octoprint.org/) を使ってKlipperをコントロールする方法もあります。
...
注意するべきこととして、この操作を行う前にRP2040チップをブートモードにする必要があります。

## Klipperの設定

次は、[プリンタ設定ファイル](#obtain-a-klipper-configuration-file) をホストにコピーしてください。
...
それからその固有名で設定ファイルを更新してください。例えば、`[mcu]` の項目を以下のように更新します:
...
Klipperがプリンターの準備完了を報告したら、 [設定の確認](Config_checks.md)に進み、設定ファイル内の定義について基本的なチェックを行います。詳しくは[概要](Overview.md)を見てください。
