...
'AXIS_TWIST_COMPENSATION_CALIBRATE [SAMPLE_COUNT=<valor>]': Inicia o assistente de calibração de torção do eixo X. 'SAMPLE_COUNT' especifica o número de pontos ao longo do eixo X para calibrar e tem o valor padrão de 3.
...
`BED_MESH_CALIBRATE [PROFILE=<name>] [METHOD=manual] [HORIZONTAL_MOVE_Z=<value>] [<probe_parameter>=<value>] [<mesh_parameter>=<value>] [ADAPTIVE=1] [ADAPTIVE_MARGIN=<value>]`: Este comando realiza a sondagem da cama (bed) utilizando pontos gerados especificados pelos parâmetros na configuração. Após a sondagem, uma malha é gerada e o movimento em Z é ajustado de acordo com a malha. A malha será salva em um perfil especificado pelo parâmetro 'PROFILE', ou 'default' se não especificado. Veja o comando PROBE para detalhes sobre os parâmetros opcionais de sondagem. Se METHOD=manual for especificado, então a ferramenta de sondagem manual é ativada - consulte o comando MANUAL_PROBE acima para detalhes sobre os comandos adicionais disponíveis enquanto esta ferramenta estiver ativa. O valor opcional 'HORIZONTAL_MOVE_Z' substitui a opção 'horizontal_move_z' especificada no arquivo de configuração. Se ADAPTIVE=1 for especificado, então os objetos definidos pelo arquivo Gcode que está sendo impresso serão usados para definir a área de sondagem. O valor opcional 'ADAPTIVE_MARGIN' substitui a opção 'adaptive_margin' especificada no arquivo de configuração.

#### BED_MESH_OUTPUT

`BED_MESH_OUTPUT PGP=[<0:1>]`: Este comando exibe os valores atuais de Z medidos e os valores atuais da malha no terminal. Se PGP=1 for especificado, as coordenadas X, Y geradas pelo bed_mesh, juntamente com seus índices associados, serão exibidas no terminal.

#### BED_MESH_MAP

`BED_MESH_MAP`: Semelhante ao BED_MESH_OUTPUT, este comando imprime o estado atual da malha no terminal. Em vez de imprimir os valores em um formato legível para humanos, o estado é serializado no formato JSON. Isso permite que os plugins do OctoPrint capturem facilmente os dados e gerem mapas de altura que aproximam a superfície da cama.
...
`BED_MESH_PROFILE LOAD=<name> SAVE=<name> REMOVE=<name>`: Este comando fornece gerenciamento de perfil para o estado da malha. LOAD irá restaurar o estado da malha a partir do perfil correspondente ao nome fornecido. SAVE irá salvar o estado atual da malha em um perfil correspondente ao nome fornecido. REMOVE irá excluir o perfil correspondente ao nome fornecido da memória persistente. Observe que, após a execução das operações SAVE ou REMOVE, o comando SAVE_CONFIG deve ser executado para tornar as alterações permanentes na memória persistente.

#### BED_MESH_OFFSET

`BED_MESH_OFFSET [X=<value>] [Y=<value>] [ZFADE=<value]`: Aplica deslocamentos X, Y e/ou ZFADE na consulta da malha. Isso é útil para impressoras com extrusores independentes, pois um deslocamento é necessário para produzir o ajuste correto de Z após a troca de ferramenta. Observe que um deslocamento ZFADE não aplica ajuste adicional em Z diretamente, ele é usado para corrigir o cálculo do 'fade' quando um deslocamento de gcode foi aplicado no eixo Z.
...
`BED_SCREWS_ADJUST`: Este comando ativará a ferramenta de ajuste dos parafusos da cama. Ele comandará o bico para diferentes locais (conforme definido no arquivo de configuração) e permitirá fazer ajustes nos parafusos da cama para que ela mantenha uma distância constante do bico.
