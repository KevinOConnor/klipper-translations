...
Последний раз тест выполнялся на коммите `08d037c6` с gcc версии `or1k-linux-musl-gcc (GCC) 9.2.0` на микроконтроллере Allwinner A64-H.
...
### Эталон скорости шага RP2040

Для RP2040 используется следующая последовательность настройки:
...
Последний раз тест был выполнен на коммите `59314d99` с gcc версии `arm-none-eabi-gcc (Fedora 10.2.0-4.fc34) 10.2.0` на плате Raspberry Pi Pico.
