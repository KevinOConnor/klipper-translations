...
1. Інші методи. `check_move()`, `get_status()`, `get_steppers`, `home()`, і `set_position()` Ці функції зазвичай використовуються для надання кінематичних специфічних перевірок. Однак на старті розробки можна скористатися котельно-платним кодом тут.
